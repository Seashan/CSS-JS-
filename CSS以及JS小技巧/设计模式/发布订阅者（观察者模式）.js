/*在JS的dom事件中就是利用的发布订阅者模式，发布订阅者模式有利于对象间的解耦，
两个相关的对象在执行相关操作时并不需要知道彼此对象的内容，如果不使用发布订阅者模式，
则A对象操作完成要执行B对象中的某项操作，则必须知道B中对象内容，执行相应的方法，
利用订阅者模式，A中执行完后，直接利用事件通知B,B自己执行就好，A不需要知道B中的内容*/
